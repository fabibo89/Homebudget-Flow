...
export default function Contracts() {
  ...
  const rowsByTab = useMemo(() => {
    ...
    /** „Bis“ = last_booking; neuestes Enddatum (Jahr/Datum) zuerst, ohne Datum zuletzt. */
    suggested.sort((a, b) => {
    ...
    <Stack spacing={2.5}>
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }} flexWrap="wrap">
        ...
        {recognizeMut.isSuccess ? (
          <Typography variant="body2" color="text.secondary">
            Vorschläge aktualisiert: {recognizeMut.data.suggestions_updated}, bestätigte Links:{' '}
            {recognizeMut.data.confirmed_links_touched}
          </Typography>
        ) : null}
      </Stack>
            ...
            <Table size="small">
              ...
              <TableBody>
                {currentRows.length === 0 ? (
                  <TableRow>
                    <TableCell
                      colSpan={tab === 'ignored' ? 7 : tab === 'suggested' ? 9 : 8}
                    >
                      <Typography variant="body2" color="text.secondary">
